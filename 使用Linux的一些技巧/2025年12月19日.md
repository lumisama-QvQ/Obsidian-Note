...
这里先附上内存指标的含义：
```
RSS (Resident)	进程当前“吃掉”的物理内存，最直观，但包含了与其他进程共享的库。

PSS (Proportional)	进程“分摊后”的内存	最准确。如果有 10 个进程共享一个 10M 的库，每个进程只算 1M。

USS (Unique)	进程“私有”的内存，即使杀死这个进程，系统也只能回收这么多内存。
```
## 原生自带：
```bash
//查看占用前十
...
选项:
  ...
  -S SOURCE, --source=SOURCE
						指定 /proc 数据源路径
```
使用`smem -rk | tail -n 10`即可友好的查看：
![](附件/Pasted%20image%2020251219140213.png)
> 注：`smem`并不在标准库中，须从AUR下载

# VSCode下为C/C++语言启用精准LSP
在VSCode默认的C语言lsp(语法检查补全系统)下，默认使用的是模糊匹配模式，这种情况下LSP会对不少错误视而不见，按以下步骤可增强LSP性能
## 1.安装Clangd插件
安装后VSCode会显示冲突，我们按提示关闭默认插件的`IntelliSense`功能
